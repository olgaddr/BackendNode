 ...
 app.get("/api/room/:number/sensors", function (req, res) {
   ...
   "SELECT sensor.sensorname FROM sensor INNER JOIN controller_sensor ON controller_sensor.id_sensor=sensor.id " +
    ...
    /*app.get("/api/room/:number/sensors/data", function (req, res) {
        db.any(
       "SELECT sensor.sensorname, datasensor.data FROM datasensor INNER JOIN (sensor INNER JOIN controller_sensor ON controller_sensor.id=sensor.id) ON controller_sensor.id=datasensor.id" +
        "WHERE controller_sensor.room=" + req.params.number + ":: varchar"
        )
        .then(function (data) {
        res.json({
